...
protected:
	...
	void printLevelOrderHelper(Node<T>* root, int level) {
		if (!root)
			return;
		if (level == 0)
			cout << root->value << " ";
	...
	void printMaxPathHelper(Node<T>* root, int height, int* value) {
		if (!root)
			return;
		else if (height > 0) 
		{
			printMaxPathHelper(root->left, height - 1, value);
			printMaxPathHelper(root->right, height - 1, value);
		}
		else if (height == 0)
		{
			*value = root->value;
	...
	void deleteTreeHelper(Node<T>* root) {
		if (!root)
			return;
...
public:
	...
	void printMaxPath() {
		...
		printMaxPathHelper(_root, treeHeight, &value);
